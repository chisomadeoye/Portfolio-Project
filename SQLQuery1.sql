...
--- Select the Dara we're going to be using
...
----Mortality Rate of Covid in Canada
...
------Looking at continet with the highest death counts
...
------Loooking at continents with highest infection rates compared to population

select continent, MAX(cast(total_cases as int)) as HighetInfectionCount, MAX((total_cases/population))*100 as InfectionPercentage
From SQLPortfolio..CovidDeaths
where continent is not null
GROUP BY continent
order by InfectionPercentage desc
...
-------Looking at Total of Vaccinated vs Population using a CTE
...
----Creating View to Stoe data for later visuilization
